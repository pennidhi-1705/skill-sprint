...
const GENERAL_CATEGORIES = [
  ...
  {
    id: "capability",
    test: (t) => /what can you help( me)? with|what do you do\??$|how can you help( me)?\??$|can you help me\??$/.test(t),
    responses: [
...
export function parseInput(rawInput) {
  ...
  if (/why/.test(text) && (/match/.test(text) || /%|percent/.test(text) || /recommend/.test(text))) {
  ...
  if (/^apply\b|i want to apply|let'?s apply|apply now|apply for (this|it)|can i apply/.test(text)) return { intent: "apply" };

  if (/how long|duration/.test(text)) return { intent: "detail_duration" };
  if (/is (this|it) remote|remote or|work mode|in[- ]person|on[- ]site|work from|where is (this|it)/.test(text)) return { intent: "detail_work_mode" };
  if (/what exactly do i (have to do|do)|what will i (be doing|do)|what does it involve|deliverable/.test(text)) return { intent: "detail_deliverables" };
  if (/can a beginner|beginner[- ]friendly|do i need experience|experience (level )?required|i don'?t know [a-z]+\.? can i still/.test(text)) return { intent: "detail_eligibility" };
  if (/required skills|skills (do i )?need|what skills does (it|this) (need|require)/.test(text)) return { intent: "detail_required_skills" };
  if (/reward|stipend|pay|how much/.test(text)) return { intent: "detail_reward" };
  if (/tell me more|more (details|info|information) about|elaborate/.test(text)) return { intent: "tell_me_more" };
  ...
  if (/opportunit|suitable for me|what'?s (out there|available)/.test(text)) return { intent: "find_opportunities", filters: {} };
  ...
  if (/skill gap|what'?s missing|what am i missing|missing (from|in) my profile|gaps? in my (skills|profile)/.test(text)) {
...
export function getHarryResponse(rawInput, student, context) {
  ...
  switch (parsed.intent) {
    ...
    default:
      return {
        message: reply(pick([
          "I'm not fully sure about that one, but I'm happy to think it through with you - tell me a bit more. I'm also useful for skills to learn, career direction, project ideas, a learning plan, or your real opportunity matches whenever you want to switch gears.",
          "Good question - let's dig into it, give me a little more detail. Or we could look at your skill gaps, a weekly learning plan, or opportunities that actually fit you right now.",
          "Tell me more about what you mean and I'll do my best with it. In the meantime I can also help with career guidance, project ideas, internship-readiness, or real SkillSprint opportunities."
        ])),
        context
      };
  }

  return { message: reply("Could you rephrase that? I want to make sure I help with the right thing.") , context };
}
...
// ---------------------------------------------------------------------------
// Real-AI provider hook (optional).
//
// getHarryResponse() above is the deterministic engine: it owns every
// opportunity/matching/application answer, always grounded in real data, and
// never needs a network call. getHarryReply() wraps it and adds ONE thing -
// for messages the deterministic engine doesn't recognize ("unknown"
// intent), it gives an optional real LLM backend (see
// harry/harryAIProvider.js + backend/server.js's POST /api/harry/chat) a
// chance to answer instead of the generic fallback line. If no AI provider
// is configured on this deployment (no AI_API_KEY), or the call fails or
// times out, Harry silently keeps the deterministic fallback text - the
// student never sees an error, and nothing here can invent opportunities,
// matches, or applications, since that whole surface never leaves the
// deterministic engine.
//
// HarryWidget should call this instead of getHarryResponse() directly.
// ---------------------------------------------------------------------------
export async function getHarryReply(rawInput, student, context) {
  const deterministic = getHarryResponse(rawInput, student, context);
  if (!student) return deterministic;

  const { intent } = parseInput(rawInput);
  if (intent !== "unknown") return deterministic;

  const aiText = await tryHarryAIProvider(rawInput, student, context, deterministic.message.text);
  if (!aiText) return deterministic;

  return {
    message: { ...deterministic.message, text: aiText },
