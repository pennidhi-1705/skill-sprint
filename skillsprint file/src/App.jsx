...
function Layout({ children }) {
  ...
  return <div className="app-shell">
    <header className="navbar">
      <Link className="brand" to="/">Skill<span>Sprint</span></Link>
      <nav className="navlinks">
        <Link to="/">Home</Link><a href="/#how">How It Works</a>
        {user?.role === "student" ? <><Link to="/opportunities">Opportunities</Link><Link to="/organizations">Organizations</Link><Link to="/network">Network</Link><Link to="/messages">Messages</Link><Link to="/student/dashboard">Dashboard</Link></>
          : user?.role === "organization" ? <><Link to="/organization/dashboard">Dashboard</Link><Link to="/organization/post-task">Post a Task</Link><Link to="/organization/applications">Applications</Link><Link to="/messages">Messages</Link><Link to="/organization/verification">Verification</Link></>
          : user?.role === "admin" ? <Link to="/admin">Internal Review</Link>
          : <><Link to="/organizations">Organizations</Link><Link to="/login?role=student">For Students</Link><Link to="/login?role=organization">For Organizations</Link></>}
      </nav>
    ...
    <main>{children}</main>
    <footer className="footer"><div><strong>SkillSprint</strong><p>Small tasks. Real experience. Local impact.</p></div><div><Link to="/opportunities">Find a Project</Link><Link to="/login?role=organization">Post a Task</Link></div><small>Prototype • Built for a hackathon</small></footer>
    <HarryWidget/>
  </div>;
}
...
function ChatShowcaseSection() {
  return <section className="section alt">
    <div className="split">
      <div><div className="eyebrow">DIRECT COMMUNICATION</div><h2>Don't just apply. Connect.</h2><p>When an organization is interested in your profile, you can continue the conversation directly inside SkillSprint — no switching to email or chat apps.</p><Link className="text-link" to="/login?role=student">Start connecting →</Link></div>
...
function Home() {
  return <><section className="hero"><div className="hero-copy"><Badge tone="accent">Where skills meet opportunity</Badge><h1>Small tasks.<br/><em>Real experience.</em><br/>Trusted connections.</h1><p>Discover verified organizations, find opportunities that match your skills, build meaningful professional connections, and grow your career — all in one place.</p><div className="hero-actions"><Link className="btn primary" to="/opportunities">Explore Opportunities →</Link><Link className="btn secondary" to="/login?role=student">Join SkillSprint</Link></div><div className="hero-proof"><span>✓ Verified organizations</span><span>✓ AI-assisted verification</span><span>✓ Direct messaging</span></div></div><div className="hero-visual"><div className="floating-card top"><span className="icon">✦</span><div><b>92% match</b><small>Frontend Developer · ABC Technologies ✓</small></div></div><div className="visual-panel"><div className="mini-label">YOUR NEXT SPRINT</div><h3>Community Data Cleanup</h3><p>Local Impact NGO ✓ · 1 day · Hybrid</p><div className="progress"><span style={{width:"87%"}}/></div><div className="visual-row"><Badge tone="success">Verified</Badge><strong>₹1,200</strong></div></div><div className="floating-card bottom"><span className="check">🎉</span><div><b>You're shortlisted</b><small>ABC Technologies wants to connect</small></div></div></div></section>
    <section className="section problem"><div className="eyebrow">THE GAP</div><h2>Great opportunities are often too small — or too easy to fake — to trust.</h2><p className="lead">Traditional internship platforms focus on long-term placements with no way to verify who's actually on the other side. SkillSprint makes short, practical work visible, backed by verified organizations and direct communication.</p><div className="task-pills">{["Website Update","Data Entry","Survey","Translation","Design","Technical Assistance"].map(x=><span key={x}>{x}</span>)}</div></section>
    <section className="section alt" id="how"><div className="eyebrow">HOW SKILLSPRINT WORKS</div><h2>Discover → Verify → Connect → Apply → Collaborate → Grow.</h2><div className="flow">{["Build your profile","Discover opportunities","Follow organizations you trust","Apply & get shortlisted","Chat directly","Complete real work","Get reviewed","Verified experience"].map((x,i)=><div className="flow-step" key={x}><span>{String(i+1).padStart(2,"0")}</span><b>{x}</b></div>)}</div></section>
    ...
    <TrustSection/>
    <ChatShowcaseSection/>
    <section className="cta"><h2>Your skills can take you further.</h2><p>Discover opportunities. Connect with organizations. Build real experience.</p><div className="button-row center"><Link className="btn primary" to="/opportunities">Explore Opportunities</Link><Link className="btn secondary" to="/login?role=student">Join SkillSprint</Link></div></section>
  </>;
}
